...
class _Block(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.conv1 = nn.Conv2d(in_channels, out_channels, 3, stride=1, padding=1, bias=True)
        self.prelu = nn.PReLU(out_channels)
        self.conv2 = nn.Conv2d(out_channels, out_channels, 3, stride=stride, padding=1, bias=True)
...
class VerifyIResNet100(nn.Module):
    ...
    def load_from_torchscript(self, ts_model):
        ...
        assert idx == len(constants), f"Expected {len(constants)} constants, consumed {idx}"
