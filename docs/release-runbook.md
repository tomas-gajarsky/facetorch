...
1. Work from a clean, protected commit. Confirm the selected ref resolves to the
   same commit as the workflow SHA; do not publish from a mutable branch head.
...
3. Verify every model record has an immutable revision, SHA-256, format, source
   provenance, weight-license decision, attribution, and limitations. Any
...
plan, exact artifact set, checksums, SBOM/provenance, and a digest-bound receipt.
...
the six-month v0.6.x critical/security-only support period. Keep a second
operator assigned before stable publication; until then, treat owner-only
operation as a release risk and rehearse recovery from the receipts.
