...
GOLDEN_REFERENCE_SCHEMA_VERSION = 1
...
def _write_torch_atomic(path: Path, value: Any) -> None:
    """Write deterministic tensor evidence without replacing different bytes."""
    _ensure_runtime_directory(path.parent)
    temporary = path.with_name(f".{path.name}.{os.getpid()}.tmp")
    try:
        with temporary.open("wb") as output:
            # A file object gives torch.save a stable archive root, so identical
            # tensor bundles produce identical bytes across staging directories.
            torch.save(value, output)
            output.flush()
            os.fsync(output.fileno())
        temporary.chmod(0o644)
        try:
            # link() publishes without the overwrite race of replace(); a
            # concurrent writer can only leave identical bytes or make us fail.
            os.link(temporary, path)
        except FileExistsError:
            if not path.is_file() or path.is_symlink():
                raise RuntimeError(
                    f"Golden reference target is not a regular file: {path}"
                )
            if _sha256(path) != _sha256(temporary):
                raise RuntimeError(
                    f"Existing golden reference differs from regenerated bytes: {path}"
                )
    finally:
        temporary.unlink(missing_ok=True)
...
def _fixed_reference_device(
    spec: Mapping[str, Any], *, required: bool = False
) -> str | None:
    reference = spec.get("validation_reference", {})
    if not isinstance(reference, Mapping):
        raise RuntimeError("Validation reference contract must be an object")
    raw_device = reference.get("device")
    if raw_device is None or not str(raw_device).strip():
        if required:
            raise RuntimeError("Persistent golden references require a fixed device")
        return None
    try:
        return str(torch.device(str(raw_device).strip().lower()))
    except (RuntimeError, ValueError) as exc:
        raise RuntimeError(
            f"Validation reference device is invalid: {raw_device!r}"
        ) from exc


def _golden_reference_contract(spec: Mapping[str, Any]) -> Dict[str, Any]:
    reference = spec.get("validation_reference", {})
    if not isinstance(reference, Mapping):
        raise RuntimeError("Validation reference contract must be an object")
    ...
    if missing:
        raise RuntimeError(
            "Persistent golden reference contract is incomplete: " + ", ".join(missing)
        )
    sha256 = str(reference["sha256"]).lower()
    if re.fullmatch(r"[0-9a-f]{64}", sha256) is None:
        raise RuntimeError("Persistent golden reference has an invalid source digest")
    batch_mode = str(reference.get("batch_mode", "native")).strip().lower()
    if batch_mode not in {"native", "per_sample"}:
        raise RuntimeError(
            f"Persistent golden reference has invalid batch mode: {batch_mode!r}"
        )
    return {
        "kind": str(reference["kind"]),
        "source": str(reference["source"]),
        "sha256": sha256,
        "device": _fixed_reference_device(spec, required=True),
        "batch_mode": batch_mode,
    }


def _golden_matrix_contract(
    batch_sizes: Sequence[int], seeds: Sequence[int], scales: Sequence[float]
) -> Dict[str, Any]:
    return {
...
def _build_golden_reference_bundle(
    ...
    *,
    ...
    reference_device = _fixed_reference_device(spec, required=True)
    assert reference_device is not None
    if reference_device.startswith("cuda") and not torch.cuda.is_available():
        raise RuntimeError(
            f"Golden reference device is unavailable: {reference_device}"
        )

    case_records: Dict[str, Any] = {}
    for case in cases:
        case_id = str(case["id"])
        x_cpu = case["x"].detach().cpu().contiguous()
        with torch.no_grad():
            output = _clone_output_cpu(ref_fn(x_cpu.to(reference_device)))
        _validate_output_contract(
            spec, output, int(case["batch"]), "Golden reference output"
        )
        case_records[case_id] = {
            ...
            "input_sha256": _output_sha256(x_cpu),
    ...
    return {
        ...
        "reference": _golden_reference_contract(spec),
        "matrix": _golden_matrix_contract(batch_sizes, seeds, scales),
        "cases": case_records,
    }


def _validate_golden_reference_bundle(
    bundle: Any,
    spec: Mapping[str, Any],
    cases: Sequence[Mapping[str, Any]],
    *,
    ...
    if not isinstance(bundle, Mapping):
        raise RuntimeError("Golden reference bundle must contain an object")
    expected_identity = {
        "schema_version": GOLDEN_REFERENCE_SCHEMA_VERSION,
        "model_id": str(spec["id"]),
        "source_cohort": str(source_cohort),
    ...
    for key, expected in expected_identity.items():
        if bundle.get(key) != expected:
            raise RuntimeError(f"Golden reference bundle has mismatched {key}")

    records = bundle.get("cases")
    if not isinstance(records, Mapping):
        raise RuntimeError("Golden reference bundle has no case matrix")
    expected_case_ids = {str(case["id"]) for case in cases}
    if set(records) != expected_case_ids:
        raise RuntimeError("Golden reference bundle case matrix is incomplete")
    ...
    for case in cases:
        ...
        if not isinstance(record, Mapping):
            raise RuntimeError(f"Golden reference case is invalid: {case_id}")
        x_cpu = case["x"].detach().cpu().contiguous()
        expected_case = {
            "batch": int(case["batch"]),
            "seed": int(case["seed"]),
        ...
        if any(record.get(key) != value for key, value in expected_case.items()):
            raise RuntimeError(f"Golden reference input changed for case {case_id}")
        output = _clone_output_cpu(record.get("output"))
        _validate_output_contract(
            spec, output, int(case["batch"]), "Golden reference output"
        )
        output_sha = _output_sha256(output)
        if record.get("output_sha256") != output_sha:
            raise RuntimeError(f"Golden reference output changed for case {case_id}")
        outputs[case_id] = output
    return outputs


def _persistent_golden_reference_outputs(
    spec: Mapping[str, Any],
    ref_fn,
    cases: Sequence[Mapping[str, Any]],
    *,
    ...
    if mode not in {"record", "reuse"}:
        raise RuntimeError(f"Unsupported golden reference mode: {mode}")
    if (
        re.fullmatch(r"\d+\.\d+", cohort) is None
        or re.fullmatch(r"\d+\.\d+", source_cohort) is None
    ):
        raise RuntimeError(
            "Golden reference cohorts must be explicit Torch minor lines"
        )
    if mode == "record":
        if cohort != source_cohort:
            raise RuntimeError(
                "Golden references may only be recorded by their declared source cohort"
            )
        bundle = _build_golden_reference_bundle(
            spec,
            ref_fn,
            cases,
        ...
        _write_torch_atomic(path, bundle)
    else:
        if not path.is_file() or path.is_symlink():
            raise RuntimeError(f"Golden reference bundle is missing: {path}")
        try:
            # Golden evidence is an untrusted staged input. Supported release
            # runtimes all provide weights_only and must never fall back to pickle.
            bundle = torch.load(str(path), map_location="cpu", weights_only=True)
        except Exception as exc:
            raise RuntimeError(f"Cannot load golden reference bundle: {path}") from exc

    outputs = _validate_golden_reference_bundle(
        bundle,
        spec,
        cases,
        batch_sizes=batch_sizes,
    ...
    return outputs, {
        "schema_version": GOLDEN_REFERENCE_SCHEMA_VERSION,
        "status": "recorded" if mode == "record" else "reused",
        "source_cohort": source_cohort,
        "sha256": _sha256(path),
...
def _allowed_state_keys(
    ...
    for key in keys:
        target = (
            allowed if any(pattern.fullmatch(key) for pattern in compiled) else rejected
        )
        target.append(key)
...
def _load_native_model(spec: Dict[str, Any]):
    ...
    if strategy == "native_state_dict":
        ...
        if not hasattr(model, "load_from_torchscript"):
            raise RuntimeError(
                f"Model {spec['class_path']} has no load_from_torchscript"
            )
...
def _verified_reused_artifact(spec: Dict[str, Any], torch_minor: str) -> Dict[str, Any]:
    ...
    try:
        ...
        artifact = next(
            item for item in model_record["artifacts"] if item.get("id") == artifact_id
        )
...
def _model_specs(torch_minor: str) -> List[Dict[str, Any]]:
    specs = [
        ...
        },
        {
            ...
            "source_path": (
                "models_local/reused/au-opengraph/" f"model-torch{torch_minor}.pt2"
            ),
...
def _safe_source_target(repo_root: Path, relative_path: str) -> Path:
    ...
    try:
        ...
        raise RuntimeError(
            f"Model source escapes repository root: {relative_path}"
        ) from exc
    return target
    ...
    *,
    ...
    for spec in specs:
        ...
        if (
            ...
            raise RuntimeError(
                f"Source contract disagrees with manifest for {model_id}"
            )
        ...
        for case_h, case_w in spatial_shapes:
            for seed in seeds:
                for scale in scales:
                    ...
                    x_randu = (
                        torch.rand((b, c, int(case_h), int(case_w)), generator=g2) * 2.0
                        - 1.0
                    ) * scale
...
def _validate_output_schema_node(
    ...
    if kind == "sequence":
        ...
        for index, (item_schema, value) in enumerate(zip(items, output)):
            _validate_output_schema_node(item_schema, value, batch, f"{path}[{index}]")
        return
    ...
    for invariant in spec.get("output_invariants", []):
        if invariant == "detector_heads":
            ...
            if anchors < 1 or any(
                ...
                raise RuntimeError(
                    "Detector heads do not share a non-empty anchor axis"
                )
...
def _validate_exported_module_impl(
    ...
    golden_reference_path: Path | None = None,
    golden_reference_mode: str = "ephemeral",
    cohort: str = "",
    golden_reference_cohort: str = "",
) -> Dict[str, Any]:
    ...
    fixed_reference_device = _fixed_reference_device(spec)
    matrix_failures = []
    ...
    observed_shapes = {
        tuple(int(value) for value in case["x"].shape[-2:]) for case in cases
    }
    required_shapes = {
    ...
    if golden_reference_mode in {"record", "reuse"}:
        if matrix_failures:
            raise RuntimeError(
                "Cannot use a persistent golden reference with an incomplete matrix"
            )
        if golden_reference_path is None or not golden_reference_cohort:
            raise RuntimeError(
                "Persistent golden references require a path and source cohort"
            )
        fixed_reference_outputs, golden_reference = (
            _persistent_golden_reference_outputs(
                spec,
                ref_fn,
                cases,
                batch_sizes=batch_sizes,
                ...
                mode=golden_reference_mode,
                cohort=cohort,
                source_cohort=golden_reference_cohort,
            )
        )
    elif golden_reference_mode == "ephemeral":
        if fixed_reference_device is not None:
            for case in cases:
                with torch.no_grad():
                    output = _clone_output_cpu(
                        ref_fn(case["x"].to(fixed_reference_device))
                    )
                _validate_output_contract(
                    spec,
                    output,
                ...
                fixed_reference_outputs[str(case["id"])] = output
            golden_reference["case_count"] = len(fixed_reference_outputs)
    else:
        raise RuntimeError(
            f"Unsupported golden reference mode: {golden_reference_mode}"
        )
    ...
    for device_name in requested_devices:
        ...
        for case in cases:
            ...
            if device_worst_case_id is None or stats["max_abs"] > device_worst_max_abs:
            ...
            case_result = {
                ...
                "variant": case["variant"],
                "input_shape": list(x.shape),
                "input_sha256": _output_sha256(case["x"]),
                ...
                "reference_execution_device": (fixed_reference_device or device_name),
            ...
            if device_name == baseline_device:
                ...
                cross_stats = _compute_diff_stats(baseline_outputs[case["id"]], exp_out)
                cross_failed = (
...
def _validate_exported_module(
    ...
    with _validation_numeric_policy() as numeric_policy:
        result = _validate_exported_module_impl(
            ...
            golden_reference_mode=golden_reference_mode,
            cohort=cohort,
            golden_reference_cohort=golden_reference_cohort,
        )
...
def _resolve_artifact_path(
    artifacts_root: Path, spec: Dict[str, Any], cohort: str
) -> Path:
...
def _run_for_specs(
    ...
    environment_lock: str | Path | None = None,
    golden_reference_root: Path | None = None,
    golden_reference_mode: str = "ephemeral",
    golden_reference_cohort: str = "",
) -> Tuple[Dict[str, Any], List[Tuple[str, str, str]]]:
    if mode not in {"export", "validate"}:
        raise RuntimeError(f"Unsupported mode: {mode}")
    if golden_reference_mode in {"record", "reuse"} and (
        golden_reference_root is None or not golden_reference_cohort
    ):
        raise RuntimeError(
            "Persistent golden references require a root and source cohort"
        )

    specs_list = list(specs)
    environment = _environment_metadata(repo_root, environment_lock)
    runtime_torch_minor = ".".join(str(torch.__version__).split("+")[0].split(".")[:2])
    exporter_arguments = {
        "mode": mode,
        ...
        "golden_reference_mode": golden_reference_mode,
        "golden_reference_cohort": golden_reference_cohort or None,
    ...
    for idx, spec in enumerate(specs_list, start=1):
        os.chdir(repo_root)
        print(
            f"[{idx}/{len(specs_list)}] {mode.title()} {spec['id']} ({spec['repo_id']})"
        )
        ...
        try:
            ...
            golden_reference_path = (
                golden_reference_root / spec["id"] / "golden-reference.pt"
                if golden_reference_root is not None
                else None
            )
            ...
            if golden_reference_path is not None:
                golden = validation["golden_reference"]
                result.update(
                    {
                        "golden_reference": str(golden_reference_path),
                        "golden_reference_sha256": golden["sha256"],
                        "golden_reference_size_bytes": golden["size_bytes"],
                    }
                )
...
def main():
    ...
    def add_common_args(p):
        ...
        p.add_argument(
            ...
            help="Root containing one immutable golden-reference bundle per model.",
        )
        p.add_argument(
            "--golden-reference-mode",
            choices=("ephemeral", "record", "reuse"),
            default="ephemeral",
            help=(
                "Use runtime-local references, record the source cohort, or reuse "
                "the already recorded source cohort."
            ),
        )
        p.add_argument(
            "--golden-reference-cohort",
            default="",
            help="Cohort that records the persistent golden reference (for example 2.6).",
        )
    ...
    golden_reference_root = (
        Path(args.golden_reference_root).resolve()
        if args.golden_reference_root
        else None
    )
    if args.golden_reference_mode in {"record", "reuse"} and (
        golden_reference_root is None or not args.golden_reference_cohort
    ):
        parser.error(
            "persistent golden references require --golden-reference-root and "
            "--golden-reference-cohort"
        )
    ...
    if args.command == "export":
        ...
        summary, failures = _run_for_specs(
            ...
            environment_lock=args.environment_lock,
            golden_reference_root=golden_reference_root,
