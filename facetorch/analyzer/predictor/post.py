...
class PostELIMAL(BasePredPostProcessor):
    def __init__(
        self,
        transform: transforms.Compose,
        device: torch.device,
        ...
        Args:
            ...
            optimize_transform (bool): Whether to optimize the transform using TorchScript.
            labels (List[str]): List of labels.
            offsets (List[float]): Offsets for valence and arousal. Defaults to [0.25, 0.05].
        """
        super().__init__(transform, device, optimize_transform, labels)
    ...
    @Timer("PostELIMAL.run", "{name}: {milliseconds:.2f} ms", logger=logger.debug)
    def run(self, preds: torch.Tensor) -> List[Prediction]:
        """Extracts
        ...
        Returns:
            List[Prediction]: List of prediction data structures containing the label logit pairs.
        """
        if isinstance(preds, tuple):
            preds = preds[0]
