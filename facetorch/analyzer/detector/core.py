...
class FaceDetector(BaseModel):
    ...
    def _scale_detection_geometry(
        ...
        if data.det.boxes.numel() > 0 and data.det.boxes.ndim == 2:
            boxes = data.det.boxes.clone()
            boxes[:, 0].mul_(scale_x)
            boxes[:, 2].mul_(scale_x)
            boxes[:, 1].mul_(scale_y)
            boxes[:, 3].mul_(scale_y)
            data.det.boxes = boxes
        if data.det.landmarks.numel() > 0 and data.det.landmarks.ndim == 2:
            landmarks = data.det.landmarks.clone()
            landmarks[:, 0::2].mul_(scale_x)
            landmarks[:, 1::2].mul_(scale_y)
            data.det.landmarks = landmarks
